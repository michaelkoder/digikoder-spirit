/* Seed local SQLite DB via direct DB access
   Usage: node scripts/seed_local.js
*/
...
function insert(item) {
  ...
  const info = stmt.run(item.title, item.url, item.type || 'video', platform, item.category || 'outils', item.description || '', JSON.stringify(keywords), null, now, now);
  console.log('Inserted', info.lastInsertRowid, item.title);
}
