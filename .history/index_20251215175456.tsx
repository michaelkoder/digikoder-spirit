...
const Header = ({ user, onLogout, activeFilter, onFilterChange, onOpenAdd }: any) => {
...
const VideoEmbed = ({ url, platform, title, onOpen }: { url: string, platform: string, title: string, onOpen?: (u: string, p: string, t?: string) => void }) => {
  if (platform === 'youtube') {
    ...
    if (videoId) {
      return (
        <div className="relative pt-[56.25%] bg-black/50 rounded-lg overflow-hidden border border-white/10 group">
          <iframe
    ...
    // Fallback: show link card when ID cannot be extracted
    return (
      <div className="aspect-video bg-gradient-to-br from-purple-900 to-black rounded-lg flex flex-col items-center justify-center p-4 border border-white/10 text-center group">
        <div className="mb-3 text-sm text-gray-300">Impossible d'intégrer directement — ouvrir sur YouTube</div>
        <a href={url} target="_blank" rel="noopener noreferrer" className="text-xs bg-white/10 hover:bg-white/20 px-3 py-1 rounded-full text-white transition-colors">
          Ouvrir sur YouTube
        </a>
      </div>
    );
  ...
  if (platform === 'facebook') {
    // Render preview card that opens modal/player when clicked
    return (
      <div onClick={() => onOpen?.(url, platform, title)} className="aspect-video bg-blue-900/10 rounded-lg flex flex-col items-center justify-center p-4 border border-blue-500/10 text-center group cursor-pointer hover:border-amber-400/50 transition-colors">
        ...
        <p className="text-sm text-gray-300 mb-2 font-serif">Lire sur Facebook</p>
        <button onClick={(e) => { e.stopPropagation(); onOpen?.(url, platform, title); }} className="text-xs bg-white/10 hover:bg-white/20 px-3 py-1 rounded-full text-white transition-colors">
          Lire
        </button>
      </div>
    );
  ...
  // Fallback for Instagram or others where embed is tricky without API keys — open modal
  return (
    <div onClick={() => onOpen?.(url, platform, title)} className="aspect-video bg-gradient-to-br from-purple-900 to-black rounded-lg flex flex-col items-center justify-center p-4 border border-white/10 text-center group cursor-pointer hover:border-amber-400/50 transition-colors">
      <div className="w-12 h-12 rounded-full bg-white/10 flex items-center justify-center mb-2 group-hover:scale-110 transition-transform">
        {platform === 'instagram' ? <Instagram className="text-pink-400" /> : <LinkIcon className="text-blue-400" />}
      </div>
      <p className="text-sm text-gray-300 mb-2 font-serif">Voir</p>
      <button onClick={(e) => { e.stopPropagation(); onOpen?.(url, platform, title); }} className="text-xs bg-white/10 hover:bg-white/20 px-3 py-1 rounded-full text-white transition-colors">
        Ouvrir
      </button>
    </div>
...
const ContentCard: React.FC<{ item: ContentItem, onOpenVideo?: (u: string, p: string, t?: string) => void }> = ({ item, onOpenVideo }) => {
...
const MasonryGrid = ({ items, onOpenVideo }: { items: ContentItem[], onOpenVideo?: (u: string, p: string, t?: string) => void }) => {
...
const VideoModal = ({ isOpen, onClose, url, platform, title }: { isOpen: boolean, onClose: () => void, url?: string, platform?: string, title?: string }) => {
  ...
  try {
    if (platform === 'facebook') {
      ...
      // Convert /p/ or /reel/ links into embed form
      const u = new URL(url);
      src = `${u.origin}${u.pathname}embed`;
    }
  } catch (e) {
    src = url;
  }
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-3xl bg-black/80 rounded-lg overflow-hidden border border-white/10 z-10">
        ...
        <div className="pt-[56.25%] relative">
          <iframe className="absolute top-0 left-0 w-full h-full" src={src} title={title} allow="autoplay; encrypted-media; clipboard-write; picture-in-picture" allowFullScreen></iframe>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="min-h-screen pb-20 relative">
      ...
      <main className="max-w-7xl mx-auto px-4">
        <MasonryGrid items={filteredItems} onOpenVideo={(u,p,t) => { setModalVideo({url:u, platform:p, title:t}); setIsVideoModalOpen(true); }} />
      </main>
