...
const VideoModal = ({ isOpen, onClose, url, platform, title }: { isOpen: boolean, onClose: () => void, url?: string, platform?: string, title?: string }) => {
  ...
  try {
    if (platform === 'facebook') {
      ...
      const u = new URL(url);
      // instagram embed URLs often end with /embed or /embed/captioned
  ...
  return (
    <div className="min-h-screen pb-20 relative">
      ...
      <main className="max-w-7xl mx-auto px-4">
        <MasonryGrid items={filteredItems.filter(i => aliveMap[i.id] !== false)} onOpenVideo={(u,p,t) => { setModalVideo({url:u, platform:p, title:t}); setIsVideoModalOpen(true); }} />
      </main>
