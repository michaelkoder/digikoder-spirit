...
const VideoEmbed = ({ url, platform, title }: { url: string, platform: string, title: string }) => {
  ...
  if (platform === 'facebook') {
    // Basic Facebook embed attempt
    const encodedUrl = encodeURIComponent(url);
    return (
      <div className="relative pt-[56.25%] bg-blue-900/20 rounded-lg overflow-hidden border border-blue-500/20">
         <iframe
  ...
  // Fallback for Instagram or others where embed is tricky without API keys
  return (
    <div className="aspect-video bg-gradient-to-br from-purple-900 to-black rounded-lg flex flex-col items-center justify-center p-4 border border-white/10 text-center group cursor-pointer hover:border-amber-400/50 transition-colors">
      ...
      <p className="text-sm text-gray-300 mb-2 font-serif">Voir sur {platform}</p>
      <a href={url} target="_blank" rel="noopener noreferrer" className="text-xs bg-white/10 hover:bg-white/20 px-3 py-1 rounded-full text-white transition-colors">
        Ouvrir le lien
      </a>
    </div>
